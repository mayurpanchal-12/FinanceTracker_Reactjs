...
import { applyFilters } from "../../filters/utils/filterUtils";
...
export default function ChartPage() {
  ...
  const buildPie = (ref, instanceRef, txType, totalId, colors) => {
    ...
    txs.forEach((tx) => {
      ...
      byCat[c] = (byCat[c] || 0) + Number(tx.amount);
  ...
  useEffect(
    () =>
      buildPie(incomeChartRef, incomeInstance, "income", "totalIncome", [
        "#16a34a",
        "#22c55e",
        "#4ade80",
        "#86efac",
  ...
  useEffect(
    () =>
      buildPie(expenseChartRef, expenseInstance, "expense", "totalExpense", [
        "#e53e3e",
        "#f87171",
        "#fca5a5",
        "#dc2626",
        ...
        <div className="pie-section__grid">
          {pieCards.map(({ id, ref, chartId, variant }) => (
            <div key={id} className={`pie-card pie-card__hover--${variant}`}>
              <p
                id={id}
                className={`pie-card__label pie-card__label--${variant}`}
              >
                Loading…
              </p>

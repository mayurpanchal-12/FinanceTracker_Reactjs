    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/8">
          <div>
            <p className="text-sm font-bold text-text-main truncate max-w-xs">
              {file.name}
            </p>
...
function FileCard({ file, onPreview, onDelete, canDelete }) {
  return (
    <div
      className="card p-4 flex flex-col gap-3 animate-fade-in group cursor-pointer hover:shadow-lg transition-all"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-light">
              Receipts, documents and images — all in one place
            </p>
          </div>
            ...
            {TABS.map((tab) => (
              ...
              >
                ...
                <span className="ml-1.5 opacity-60">
                  {tab === "all"
                    ? files.length
                    : files.filter((f) => f.type === tab).length}
                </span>
              </button>
            ))}
          </div>

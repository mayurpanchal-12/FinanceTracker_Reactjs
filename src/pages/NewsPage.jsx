    ...
    <div className="news-page">
      <section className="news-section">
        ...
        <div className="news-header animate-fade-in">
          ...
          <p className="news-header__subtitle">Global market headlines powered by Alpha Vantage</p>
        ...
        {isLoading ? (
          ...
          <div className="news-grid">
            {news.map((article, index) => {
              ...
              return (
                <article key={article.url || index} className="card news-card group">
                  ...
                  <div className="news-card__top-bar">
                    ...
                    {article.overall_sentiment_label && (
                      <span className={`news-card__sentiment sentiment--${variant}`}>
                        <span className={`news-card__sentiment-dot sentiment-dot--${variant}`} />
                        {article.overall_sentiment_label}
                      </span>
                    )}
                  </div>
                  ...
                  {article.ticker_sentiment?.length > 0 && (
                    <div className="news-card__tickers">
                      {article.ticker_sentiment.slice(0, 4).map((t, i) => (
                        <span key={i} className="news-card__ticker bg-primary/8">{t.ticker}</span>
                      ))}
                    </div>
                  )}

                  <a href={article.url} target="_blank" rel="noopener noreferrer" className="news-card__cta">
                    Read Full Article →
                  </a>
          ...
          <div className="card news-empty">
            ...
            <p className="news-empty__subtitle">Check back later for the latest market updates.</p>
          </div>
        )}
